...
export default class TaskSettings extends PopupMenu.PopupMenuSection {
    ...
    constructor(task, spentTime) {
        ...
        this._descBtn   = new St.Button({ label: Utils.addNewLines(task.description),
                                          style_class: 'description-btn' });
        this._descEntry = new St.Entry({ text: task.description,
                                         style_class: 'description-label' });
        this._descEntry.hide();
        descBox.add_child(this._descBtn);
        descBox.add_child(this._descEntry);
        this.addMenuItem(descBox);

        this._descBtn.connect('clicked', () => {
            this._descBtn.hide(); this._descEntry.show();
            this._descEntry.grab_key_focus();
        });
        const saveDesc = () => {
            this._descEntry.hide(); this._descBtn.show();
        ...
        this._descEntry.clutter_text.connect('key-press-event',
            (_o,e)=>{ if ([KEY_RETURN,KEY_ENTER].includes(e.get_key_symbol())) saveDesc(); });
    ...
    _updateWeek() {
        /* you can add logic here to recompute per‑day fields */
